...
(«Claude бы спросил, но мне всё равно не нужно уведомление»).
`host/src/claude-permissions.js` (`isBlanketlyAllowed`) сузили ещё
сильнее: раньше он держал свою копию `permissions.ask`/`allow` из
`~/.claude/settings.json` и перепроверял ей каждый вызов — на практике
это дважды тихо гасило настоящий запрос на решение (составная
Bash-команда внутри широкого allow-правила; `mkdir` по пути, который
Claude Code отдельно пометил «чувствительным» — это вообще другая
проверка, не про ask/allow). Раз `PermissionRequest` сам по себе уже
стреляет только когда решение реально нужно, своя копия правил была
избыточна в лучшем случае и вредна в худшем. Сейчас `isBlanketlyAllowed`
проверяет только `permission_mode` (`bypassPermissions`/`auto`/`dontAsk`
— Claude Code в этих режимах в принципе ничего не спрашивает — и
`acceptEdits` для Edit/Write/NotebookEdit/MultiEdit), без чтения
`settings.json`.
