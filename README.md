...
Русский, английский, испанский. При первой установке берётся язык интерфейса
Chrome; дальше меняется переключателем в правом верхнем углу попапа.

Язык один на все поверхности сразу: попап, системные уведомления, сообщения
на телефон и ответы Telegram-бота. Технически это одно поле `locale` в
`routing.json` — расширение его туда пишет, хост читает для сообщений на
телефон и передаёт релею вместе с событием. Так тост в браузере и
уведомление на телефоне про одно и то же событие не оказываются на разных
языках.

Строки лежат отдельно у каждой части: `extension/i18n/`, `host/src/i18n/`,
`relay/src/i18n/`. Чтобы добавить язык — по каталогу в каждую из трёх папок,
код языка в `SUPPORTED` и чип в попапе; проверка полноты каталогов есть в
тестах (`npm test` в `host/`).
